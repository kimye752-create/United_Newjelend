"""Net Pharmacy NZ (netpharmacy.co.nz) 소매 크롤러 — 카테고리 ID 직접 타기팅.

수집 전략:
  카테고리 엔드포인트를 직접 호출하여 의약품 목록 수집 (전체 스파이더링 금지)
  INN 키워드로 제품명 필터링 후 nz_parser.py 전달

Net Pharmacy는 온라인 약국으로 만성질환 치료제·보조제 할인가,
원격 처방 연계 비급여 소매가 데이터를 제공.
"""
...
BASE_URL = "https://www.netpharmacy.co.nz"
RATE_LIMIT_DELAY = 2.0

CATEGORY_ENDPOINTS: dict[str, str] = {
    "prescription":  f"{BASE_URL}/prescription-medicines",
    "heart-health":  f"{BASE_URL}/health/heart-health",
    "cholesterol":   f"{BASE_URL}/health/cholesterol",
    "diabetes":      f"{BASE_URL}/health/diabetes",
...
def _parse_netpharmacy_listing(html: str, category: str) -> list[NetPharmacyRaw]:
    soup = BeautifulSoup(html, "html.parser")
    items: list[NetPharmacyRaw] = []

    for card in soup.select(
        ".product, .product-item, .product-card, "
        "[class*='product-'], article, "
        "[class*='ProductCard'], [class*='product-listing']"
    ):
        ...
        price_el = card.select_one(
            ...
            "[class*='price-main'], [class*='sale-price']"
        )
        if not price_el:
            continue
...
def _filter_by_inn(items: list[NetPharmacyRaw], inn_name: str) -> list[NetPharmacyRaw]:
...
async def crawl_netpharmacy(
        ...
        if not raw_items:
            for cat_name, cat_url in CATEGORY_ENDPOINTS.items():
                ...
                for page in range(1, max_category_pages + 1):
                    ...
                    page_items = _parse_netpharmacy_listing(html, cat_name)
                    if not page_items:
                        break
    ...
    for raw in filtered:
        drug = await parse_drug_text(
            ...
            source_site="netpharmacy",
            source_url=raw.url,
        )
        if drug and raw.promo_price_nzd:
